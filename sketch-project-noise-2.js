...
const sketch = () => {
  return ({ context, width, height, frame }) => {
    ...
    //   const scale = (n + 1) / 2 * 30;
    //   const scale = (n * 0.5 + 0.5) * 30;
    const scale = math.mapRange(n, -1, 1, 1, 16);
    //value, inputMin, inputMax, outputMin, outputMax, clamp = false)
...
// Drawing circles
    context.beginPath();
    context.arc(0, 0, scale, 0, Math.PI * 2); // Replace the triangle lines with an arc command
    context.closePath();
    context.fill();
    context.stroke();
    context.restore();
...
//npx canvas-sketch sketch-project-noise-2.js --open
